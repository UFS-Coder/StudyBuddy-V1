...
BEGIN
    -- Get the user ID for test@kovsies.ufs.ac.za
    SELECT id INTO test_user_id FROM auth.users WHERE email = 'test@kovsies.ufs.ac.za';
    
    IF test_user_id IS NULL THEN
        RAISE EXCEPTION 'User test@kovsies.ufs.ac.za not found';
    END IF;
    ...
    -- Effort & Progress for all subjects
    INSERT INTO grades (subject_id, user_id, grade, type, weight, description, date)
    VALUES 
    ...
    RAISE NOTICE 'Successfully inserted German Gymnasium seed data for user test@kovsies.ufs.ac.za';
END $$;